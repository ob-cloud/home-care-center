        ...
        <van-tabs v-model="active" animated>
          <van-tab :title="$t('login.title')">
            ...
            <van-cell-group v-show="!isShowSMSLogin">
              <van-field
                v-model="login_userName" required clearable :label="$t('login.phoneNumber')" maxlength="11"
                @click.stop=" changeImage(0)" :error-message="phoneNumberRight?'':$t('login.phoneNumberNotCorrect')"
                :placeholder="$t('login.phoneInput')"
              />
              <van-field
                v-model="login_password" type="password" :label="$t('login.pass')"
                :placeholder="$t('login.passTip')" required @click.stop="changeImage(1)" autocomplete
              />
              <van-field v-model="imgCaptcha" center clearable maxlength="4" :placeholder="$t('login.varify')">
                <img class="verificationImage" src="http://demo.itlike.com/web/xlmc/api/captcha" alt="captcha" @click.prevent="getCaptcha" ref="imgCaptcha" slot="button">
              </van-field>
              <!-- 手机号快捷登录 -->
            </van-cell-group>
            <van-cell-group v-show="isShowSMSLogin">
              <van-field
                v-model="login_phone" required clearable maxlength="11" :label="$t('login.phoneNumber')"
                :placeholder="$t('login.phoneInput')"
                :error-message="phoneNumberRight?'':$('login.phoneNumberNotCorrect')"
              />
              <van-field
                center clearable required :label="$t('login.varify')" maxlength="6" v-model="smsCaptcha"
                :placeholder="$t('login.varify')"
              >
                <van-button
                  slot="button" size="small" type="primary" v-if="!countDown" :disabled="captchaDisable"
                  @click="sendVerifyCode"
                >
                  {{ $t('login.sendVerify') }}
                </van-button>
                <van-button slot="button" size="small" type="primary" disabled="" v-model="smsCaptcha" v-else>
                  {{ $t('login.hasSend') }}{{ countDown }}s
                </van-button>
              </van-field>
            </van-cell-group>
            <van-button type="info" size="large" style="margin-top:1rem" @click="login">{{ $t('login.login') }}
            </van-button>
          ...
          <van-tab :title="$t('login.resgin')">
            <van-cell-group>
              <van-field v-model="register_userName" clearable maxlength="11" :label="$t('login.phoneNumber')" :placeholder="$t('login.phoneInput')" required />
              <van-field v-model="register_pwd" type="password" :label="$t('login.pass')" :placeholder="$t('login.passTip2')" required />
            </van-cell-group>
